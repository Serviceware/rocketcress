...
public class SettingsBase
{
        ...
        where T : SettingsBase
    {
        ...
        {
            var otherSettingsToken = obj[nameof(OtherSettings)];
            if (otherSettingsToken != null)
            {
                foreach (var property in otherSettingsToken.Children<JProperty>().ToList())
                {
...
#pragma warning restore SA1414 // Tuple types in signatures should have element names
    ...
    public virtual bool TryGet<T>(string settingKey, out T? value)
    {
        if (OtherSettings.ContainsKey(settingKey))
        {
            ...
            if (Conversions.TryConvert(objValue, out T? convertedValue))
            {
...
#pragma warning restore SA1414 // Tuple types in signatures should have element names
    ...
    private static class Conversions
    {
        ...
        public static bool TryConvert<T>(object? objValue, out T? value)
        {
        ...
        private static bool GetBool(object? value)
        {
            ...
            if (value is bool boolValue)
                ...
                result = longValue != 0;
            else if (value is not null && long.TryParse(GetString(value), out longValue))
                result = longValue != 0;
            else if (value is not null)
