...
internal static class SymbolExtensions
{
    ...
    public static bool IsAssignableToAny(this INamedTypeSymbol symbol, string baseTypeName1, string baseTypeName2)
    {
        var current = symbol.BaseType;
        while (current is not null)
        {
            var currentDisplayString = current.ToDisplayString();
            if (currentDisplayString == baseTypeName1 || currentDisplayString == baseTypeName2)
                return true;
            current = current.BaseType;
        }
    ...
    public static bool TryGetAttribute(this ISymbol symbol, string attributeTypeName, out AttributeData attribute)
    {
        ...
        {
            if (attributeData.AttributeClass?.ToDisplayString() == attributeTypeName)
            {
